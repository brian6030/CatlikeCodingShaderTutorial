...
public class BloomEffect : MonoBehaviour
{
    ...
    {
        // Initialize the bloom material if not already created
        if (bloom == null)
        {
        ...
        // Calculate parameters for the bloom filter
        ...
        // Set parameters in the bloom shader
        int width = source.width / iterations ;
        int height = source.height / iterations ;
        RenderTextureFormat format = source.format;

        // Create an array to hold temporary render textures
        RenderTexture[] textures = new RenderTexture[16];

        // Apply the bloom effect using blur passes
        ...
        // Apply bloom effect or debug information based on flag
        if (debug)
        {
